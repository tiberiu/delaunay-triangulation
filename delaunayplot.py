...
class Navmesh:
    ...
    def draw(self, subplotNumber, title):
        ...
        rangeX = [min([point.x for point in self.points]) - 1,
                  max([point.x for point in self.points]) + 1]

        rangeY = [min([point.y for point in self.points]) - 1,
                  max([point.y for point in self.points]) + 1]
